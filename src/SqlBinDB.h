...
#define DBFILE_READ(f, buf, size) f.read(buf, size)
#define DB_FILE_WRITE(f, buf, size) f.write(buf, size)
#define DB_FILE_SEEK(f, offset) f.seek(offset)
#define DB_FILE_CLOSE(f) f.close()
#define DB_FILE_FLUSH(f) f.flush()
#define DB_LOG_PRINT Serial.printf
#define DB_CLOSE_FILE(f) (f).close()
#define DB_READ_HEADER(f, h) ((f).read((uint8_t *)(h), sizeof(DBHeader)) == sizeof(DBHeader))
#else
#define DB_FILE_PTR FILE *
#define DB_FILE_READ(f, buf, size) fread(buf, size, 1, f)
#define DB_FILE_WRITE(f, buf, size) fwrite(buf, size, 1, f)
#define DB_FILE_SEEK(f, offset) (fseek(f, offset, SEEK_SET) == 0)
#define DB_FILE_CLOSE(f) fclose(f)
#define DB_FILE_FLUSH(f) fflush(f)
#define DB_LOG_PRINT printf
#define DB_CLOSE_FILE(f) fclose(f)
#define DB_READ_HEADER(f, h) (fread((h), sizeof(DBHeader), 1, (f)) == 1)
#endif
